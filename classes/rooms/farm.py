...
class Room_Farm(ShipRoom):
    ...
    def _plant_farm(self):
        ...
        if not self.has_manager():
            self.ship._add_log(LOG_TYPE_ROOMS,LOG_LEVEL_HIGH,str(self.name)+" needs manager to process farm.")
            return False

        if len(self.get_all_employees()) < Room_Farm.INIT_MIN_NUM_EMPLOYEES:
            self.ship._add_log(LOG_TYPE_ROOMS,LOG_LEVEL_HIGH,str(self.name)+" needs at least "+str(Room_Farm.INIT_MIN_NUM_EMPLOYEES)+" employees to process farm.")
            return False

        unmet_criteria = self.ship.get_unmet_criteria(Room_Farm.INIT_DEFAULT_PLANT_REQ_ITEMS)

        if unmet_criteria:
            self.ship._add_log(LOG_TYPE_ROOMS,LOG_LEVEL_HIGH,"Cannot process farm. Requirments not met: "+str(unmet_criteria))
            return False
        else:
            self.ship.remove_items(Room_Farm.INIT_DEFAULT_PLANT_REQ_ITEMS)
            self.ship._add_log(LOG_TYPE_ROOMS,LOG_LEVEL_LOW,str(self.item_type)+" farm planted.")
    ...
    def step_daily(self):
        ...
        if self.ready_to_plant == True:
            ...
            if self.days_till_harvest <= 0:
                ...
                if unmet_criteria:
                    self.ship._add_log(LOG_TYPE_ROOMS,LOG_LEVEL_HIGH,"Cannot grow farm. Requirments not met: "+str(unmet_criteria))
                    self._dec_yield()
                    return False
                else:
                    self._inc_yield()
    ...
    def _harvest(self):
        ...
        if not self.has_manager():
            self.ship._add_log(LOG_TYPE_ROOMS,LOG_LEVEL_HIGH,str(self.name)+" needs manager to harvest farm.")
            return False

        if len(self.get_all_employees()) < Room_Farm.INIT_MIN_NUM_EMPLOYEES:
            self.ship._add_log(LOG_TYPE_ROOMS,LOG_LEVEL_HIGH,str(self.name)+" needs at least "+str(Room_Farm.INIT_MIN_NUM_EMPLOYEES)+" employees to harvest farm.")
            return False
        ...
        if unmet_criteria:
            self.ship._add_log(LOG_TYPE_ROOMS,LOG_LEVEL_HIGH,"Cannot harvest. Requirements not met: "+str(unmet_criteria))
            return False
        else:
            self.ship.remove_items(Room_Farm.INIT_DEFAULT_HARVEST_REQ_ITEMS)
            self.ship._add_log(LOG_TYPE_ROOMS,LOG_LEVEL_LOW,"harvest time")
